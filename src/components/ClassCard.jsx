...
export function ClassCard({ item, onReserve }) {
  ...
  const isUnlimited = isUnlimitedSchedule(item)
  ...
  return (
    <Card as="article" className="p-4" variant="standard">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-2xl font-black text-white">{item.timeLabel || formatScheduleTime(item)}</p>
          ...
          <p className="mt-1 text-xs font-black uppercase text-white/60">{isUnlimited ? 'Acceso abierto · sin límite' : `Máximo ${maxSpots} alumnos`}</p>
        </div>
        <Badge state={isFull ? 'full' : 'available'}>{isUnlimited ? 'Acceso abierto' : isFull ? 'Clase completa' : `${item.spots} cupos`}</Badge>
      </div>
