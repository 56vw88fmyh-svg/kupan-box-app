...
export function AppShell({ title, eyebrow, children }) {
  return (
    <div className="k-app-shell overflow-x-hidden text-kupan-bone">
      <header className="k-app-header sticky top-0 z-20 border-b border-white/10 bg-kupan-black/90 px-4 pb-3 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
          ...
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-kupan-ember/50 bg-black/45 p-1.5 shadow-[0_10px_24px_rgba(0,0,0,0.35)]">
            <img className="h-full w-full object-contain" src={isotipoKupan} alt="KUPAN" />
          </div>
        </div>
      </header>
      <main className="k-app-main mx-auto max-w-5xl py-4 sm:px-4 sm:py-5">{children}</main>
    </div>
  )
}
