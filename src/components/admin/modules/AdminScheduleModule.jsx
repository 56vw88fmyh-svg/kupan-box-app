  ...
  return (
    <AdminSection eyebrow="Horarios" title="Clases del box">
      ...
      {scheduleItems.map((item) => (
        <SmallRow
          key={item.id}
          title={`${item.class_name} · ${formatScheduleTime(item) || toTime(item.time)}`}
          meta={`Dia ${item.day_of_week} · ${item.active ? 'Activo' : 'Inactivo'}`}
          detail={`Coach ${item.coach ?? 'KUPAN'} · ${item.unlimited_capacity ? 'acceso abierto sin límite' : `${item.max_spots} cupos`}`}
          action={(
