...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'admin_security_audit_method_check'
      and conrelid = 'public.admin_security_audit'::regclass
  ) then
    alter table public.admin_security_audit
      add constraint admin_security_audit_method_check
      check (method in ('recovery_email', 'temporary_password'));
  end if;
end $$;
